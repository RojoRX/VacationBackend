...
export class ReportsController {
  ...
  @Get('vacations/global')
  @ApiOperation({ summary: 'Reporte global de vacaciones' })
  @ApiQuery({ name: 'from', required: false, type: String, description: 'Fecha inicio (YYYY-MM-DD)' })
  @ApiQuery({ name: 'to', required: false, type: String, description: 'Fecha fin (YYYY-MM-DD)' })
  @ApiQuery({ name: 'year', required: false, type: Number, description: 'Año (opcional)' })
  @ApiQuery({ name: 'month', required: false, type: Number, description: 'Mes (1-12, opcional)' })
  @ApiQuery({ name: 'employeeType', required: false, type: String, description: 'Tipo de empleado (ADMINISTRATIVO, DOCENTE, ALL)' })
  @ApiQuery({ name: 'status', required: false, type: [String], description: 'Estados de vacaciones filtrados (PENDING, AUTHORIZED, POSTPONED, DENIED, SUSPENDED)' })
  @UsePipes(new ValidationPipe({ transform: true }))
  async generateGlobalVacationReport(
    @Query() filter: ReportFilterDto & { from?: string; to?: string; status?: string[] },
    @Res() res: Response
  ) {
    try {
      const buffer = await this.reportsService.generateGlobalVacationReport({
        from: filter.from,
        to: filter.to,
        year: filter.year,
        month: filter.month,
      ...
      const filename = `reporte_global_vacaciones_${filter.year || 'todos'}_${filter.month || 'todos'}.xlsx`;
      ...
      console.error('Error al generar reporte global de vacaciones:', error);
      res.status(500).json({
        message: 'Error al generar el reporte global de vacaciones',
        error: error.message,
      });
    }
  }
